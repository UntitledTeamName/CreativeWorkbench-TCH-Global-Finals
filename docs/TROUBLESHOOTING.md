# CharacterOS — Troubleshooting Guide

## Common Issues & Diagnoses

### Port 8765 Already In Use
- **Behavior**: The server automatically probes ports `8765-8770`. If `8765` is occupied by another application, CharacterOS binds to the first free port in the range.
- **Resolution**: Check active port via `characteros status` or `python -m story_universe_architect.cli status`.
...
- **Resolution**:
  ...
  characteros stop
  ```
  Or kill the PID recorded in `~/.characteros/characteros.pid` (or `~/.story-universe-architect/sua.pid`).
...
- **Resolution**: Clear `~/.characteros/releases/` and re-acquire, or install wheel manually with `pip install`.
